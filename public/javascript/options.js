...
const changeStyle = {
  ...
  innerCircleConfigs: function () {
    document
      .querySelectorAll('.inner-circle')[0]
      .classList.toggle('move-inner-circle');
  ...
  changeWeatherIconStyle: function () {
    document
      ...
      .classList.toggle('weather-icon-light');
  },
};
  ...
  forEventListener: {
    checkIfTransparentAndExec: function () {
      if (isGlassed == false) {
        rgbOn = !rgbOn;
        document
          .querySelectorAll('.inner-circle')[2]
          .classList.toggle('move-inner-circle');
  ...
  forChangingRGB: {
    ...
    changeShadowToRandom: function () {
      ...
      document.querySelectorAll('.card').forEach((card) => {
        card.setAttribute(
          'style',
          `box-shadow: rgb(${r},${g},${b}) 0px 1px 1px, rgb(${r},${g},${b}) 0px 2px 2px, rgb(${r},${g},${b}) 0px 4px 4px, rgb(${r},${g},${b}) 0px 8px 8px, rgb(${r},${g},${b}) 0px 16px 16px;`
        );
      });
    },
    shadowToNormal: function () {
      document.querySelectorAll('.card').forEach((card) => {
        card.setAttribute(
          'style',
          `box-shadow: rgba(0, 0, 0, 0.4) 0px 1px 1px, rgba(0, 0, 0, 0.4) 0px 2px 2px, rgba(0, 0, 0, 0.4) 0px 4px 4px, rgba(0, 0, 0, 0.4) 0px 8px 8px, rgba(0, 0, 0, 0.4) 0px 16px 16px;`
        );
      });
    },
  },
...
let isGlassed = false;
document
  .querySelector('.toggle-transparent')
  .addEventListener('click', function () {
    isGlassed = !isGlassed;
    ...
    if (
      document
        .querySelectorAll('.inner-circle')[2]
        .classList.contains('move-inner-circle')
    ) {
      document
        .querySelectorAll('.inner-circle')[2]
        .classList.remove('move-inner-circle');
      rgbOn = false;
...
const glass = {
  forToggleBtn: {
    moveInnerCircle: function () {
      document
        .querySelectorAll('.inner-circle')[1]
        .classList.toggle('move-inner-circle');
    },
  },
  forSwitchToGlass: {
...
rangeSliders[0].addEventListener('input', function () {
  let resize = (() => {
    content.style.width = `${70 - (this.value - 1) - this.value * 3}%`;
  })();
});
rangeSliders[1].addEventListener('input', function () {
  let resize = (() => {
    content.style.gap = `${2 - Math.abs(this.value - 1.9)}rem`;
  })();
});
